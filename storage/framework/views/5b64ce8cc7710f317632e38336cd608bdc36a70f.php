...
<?php if(auth()->check()): ?>
    ...
    for ( i = 0; i < allEditors.length; i++) {
        ...
        ClassicEditor.create(allEditors[i], {
                ...
                ckfinder: {
                    uploadUrl: '<?php echo e(route('admin.upload.image',['_token' => csrf_token() ])); ?>',
                    options: {
                        resourceType: 'Images'
                    }, 
                },
    ...
    notificationDropdown.addEventListener('show.bs.dropdown', function() {
        $.ajax({
            method: "POST",
            url: "<?php echo e(route('admin.notifications.see')); ?>",
    ...
    function get_notifications() {
        $.ajax({
            method: "GET",
            url: "<?php echo e(route('admin.notifications.ajax')); ?>", 
            success: function(data, textStatus, xhr) {

                favicon.badge(data.notifications.response.count_unseen_notifications);

                if (data.alert) {
                    var audio = new Audio('<?php echo e(asset("/sounds/notification.wav")); ?>');
                    audio.play();
                }  
    ...
    <?php if($unreadNotifications!=session('seen_notifications') && $unreadNotifications!=0): ?>
        <?php
...
<?php /**PATH /opt/lampp/htdocs/laravel/dashboard/resources/views/layouts/scripts.blade.php ENDPATH**/ ?>